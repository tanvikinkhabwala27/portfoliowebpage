...
export function findRelationships(sets:Dataset[]):Relationship[]{ const out:Relationship[]=[]; for(let a=0;a<sets.length;a++)for(let b=a+1;b<sets.length;b++){let best:{lc:string;rc:string;score:number;overlap:number}|null=null;for(const lc of sets[a].columns)for(const rc of sets[b].columns){const lv=new Set(sets[a].rows.map(r=>norm(r[lc]||'')).filter(Boolean));const rv=new Set(sets[b].rows.map(r=>norm(r[rc]||'')).filter(Boolean));if(!lv.size||!rv.size)continue;const matches=[...lv].filter(v=>rv.has(v)).length;const overlap=matches/Math.min(lv.size,rv.size);const nameScore=norm(lc)===norm(rc)?0.35:(norm(lc).includes(norm(rc))||norm(rc).includes(norm(lc))?0.18:0);const score=overlap+nameScore;if(!best||score>best.score)best={lc,rc,score,overlap};}if(best&&best.overlap>=.2){const strength=best.score>=1?'Strong':best.score>=.55?'Likely':'Unresolved';out.push({left:sets[a].name,right:sets[b].name,leftColumn:best.lc,rightColumn:best.rc,strength,overlap:best.overlap,consequence:`${Math.round((1-best.overlap)*100)}% of distinct key values are unmatched; related analysis may classify some records as unknown.`});}}return out.sort((x,y)=>y.overlap-x.overlap); }
